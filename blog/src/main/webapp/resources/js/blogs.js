...
app.controller("HomeController", [ '$scope', '$http', '$rootScope', function($scope, $http, $rootScope) {
	   ...
	   $scope.isLoggedIn = function() {
		      ...
		      $http.get('/blog/account/user/checklogin')
		...
		$scope.search = function(skip) {
			...
			var res = $http.post('/blog/product/search', searchObj);
			res.success(function(data, status, headers, config) {
...
app.config([ '$routeProvider', function($routeProvider) {
	...
	// Home
	.when("/", {
		templateUrl : "resources/partials/blog-home-2.html",
		controller : "PageCtrl"
	})
	// Pages
	.when("/about", {
		...
		templateUrl : "resources/partials/blog-home-1.html",
		controller : "PageCtrl"
	})
	...
	// Blog
	.when("/blog", {
		templateUrl : "resources/partials/post.html",
		controller : "BlogCtrl"
	}).when("/blog/post", {
		templateUrl : "resources/partials/post.html",
		controller : "BlogCtrl"
...
app.controller('myCtrl', ['$http','$scope', 'fileUpload', function($http, $scope, fileUpload){
    ...
    $scope.uploadFile = function(){
        ...
        console.log('file is ' );
        console.dir(file);
        var uploadUrl = "/blog/user/addproductimage";
		var productObj = {
				productName : $scope.productName,
				productDescription : $scope.productDescription
			};
		var res = $http.post('/blog/user/createproduct', productObj);
		res.success(function(data, status, headers, config) {
			$scope.message = data;
...
app.controller('CustomDesignController', ['$http','$scope', 'fileUpload', function($http, $scope, fileUpload){
    ...
    $scope.uploadFile = function(){
        ...
        console.log('file is ' );
        console.dir(file);
        var uploadUrl = "/blog/user/addcustomdesignimage";
		var customDesignObj = {
				designName : $scope.designName,
				designDescription : $scope.designDescription
			};
		var res = $http.post('/blog/user/addcustomdesign', customDesignObj);
		res.success(function(data, status, headers, config) {
			$scope.message = data;
			$scope.productId = data._id;
			$scope.inputEmail = '';
	...
	$scope.searchDesign = function(skip) {
		...
		var res = $http.post('/blog/design/search', searchObj);
		res.success(function(data, status, headers, config) {
			for (var int = 0; int < data.length; int++) {
